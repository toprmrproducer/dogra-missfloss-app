...
export default async function Home() {
  ...
  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", background: "#FFFFFF", color: "#0F172A", minHeight: "100vh" }}>
      ...
      {/* Nav */}
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 40px", borderBottom: "1px solid rgba(15,23,42,0.08)" }}>
        <span style={{ fontSize: 20, fontWeight: 700, color: "#0891B2", letterSpacing: "-0.5px" }}>Miss Floss</span>
        <div style={{ display: "flex", gap: 12 }}>
          {loggedIn ? (
            <Link href="/workflow" style={{ padding: "8px 24px", borderRadius: 8, background: "#0891B2", color: "#fff", textDecoration: "none", fontSize: 14, fontWeight: 600 }}>
              Go to Dashboard
            </Link>
          ) : (
            <>
              <Link href="/auth/login" style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.2)", color: "#0F172A", textDecoration: "none", fontSize: 14 }}>Log in</Link>
              <Link href="/auth/signup" style={{ padding: "8px 20px", borderRadius: 8, background: "#0891B2", color: "#fff", textDecoration: "none", fontSize: 14, fontWeight: 600 }}>Get Started</Link>
            </>
          )}
        </div>
      </nav>
      ...
      <section style={{ textAlign: "center", padding: "100px 40px 80px" }}>
        <div style={{ display: "inline-block", background: "rgba(8,145,178,0.10)", border: "1px solid rgba(8,145,178,0.4)", borderRadius: 100, padding: "6px 18px", fontSize: 13, color: "#0891B2", marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: "clamp(36px, 6vw, 72px)", fontWeight: 800, lineHeight: 1.1, margin: "0 0 24px", letterSpacing: "-2px", maxWidth: 800, marginInline: "auto" }}>
          Your phone lines,<br />
          <span style={{ color: "#0891B2" }}>never silent again</span>
        </h1>
        <p style={{ fontSize: 18, color: "rgba(15,23,42,0.6)", maxWidth: 560, margin: "0 auto 40px", lineHeight: 1.6 }}>
          AI voice agents that handle inbound calls, book appointments, and follow up outbound. 24/7 without a human.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          {loggedIn ? (
            <Link href="/workflow" style={{ padding: "14px 32px", borderRadius: 10, background: "#0891B2", color: "#fff", textDecoration: "none", fontSize: 16, fontWeight: 700 }}>
              Open Dashboard
            </Link>
          ) : (
            <>
              <Link href="/auth/signup" style={{ padding: "14px 32px", borderRadius: 10, background: "#0891B2", color: "#fff", textDecoration: "none", fontSize: 16, fontWeight: 700 }}>
                Start for free
              </Link>
              <Link href="/auth/login" style={{ padding: "14px 32px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.2)", color: "#0F172A", textDecoration: "none", fontSize: 16 }}>
                Sign in
              </Link>
            </>
          )}
      ...
      <section style={{ padding: "60px 40px", maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
          {[
            ...
            <div key={f.title} style={{ background: "rgba(8,145,178,0.04)", border: "1px solid rgba(15,23,42,0.08)", borderRadius: 14, padding: "28px 24px" }}>
      ...
      <section style={{ padding: "60px 40px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20 }}>
          {[
            ...
            <div key={p.name} style={{ background: p.highlight ? "rgba(8,145,178,0.10)" : "rgba(8,145,178,0.04)", border: `1px solid ${p.highlight ? "#0891B2" : "rgba(15,23,42,0.08)"}`, borderRadius: 14, padding: "32px 24px" }}>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: "20px 0 28px", display: "flex", flexDirection: "column", gap: 10 }}>
                {p.features.map((f) => <li key={f} style={{ fontSize: 14, color: "rgba(15,23,42,0.7)", display: "flex", gap: 8, alignItems: "center" }}><span style={{ color: "#0891B2" }}>+</span>{f}</li>)}
              </ul>
              <Link href={loggedIn ? "/workflow" : "/auth/signup"} style={{ display: "block", textAlign: "center", padding: "12px", borderRadius: 8, background: p.highlight ? "#0891B2" : "rgba(15,23,42,0.08)", color: "#0F172A", textDecoration: "none", fontSize: 14, fontWeight: 600 }}>
      ...
      <section style={{ textAlign: "center", padding: "80px 40px 100px" }}>
        ...
        <Link href={loggedIn ? "/workflow" : "/auth/signup"} style={{ padding: "16px 40px", borderRadius: 10, background: "#0891B2", color: "#fff", textDecoration: "none", fontSize: 17, fontWeight: 700 }}>
          {loggedIn ? "Open Dashboard" : "Create your free account"}
        </Link>
      </section>

      {/* Footer */}
      <footer style={{ borderTop: "1px solid rgba(15,23,42,0.08)", padding: "24px 40px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
