...
export const useWebSocketRTC = ({ workflowId, workflowRunId, accessToken, initialContextVariables, onNodeTransition }: UseWebSocketRTCProps) => {
    ...
    // Get WebSocket URL. Local dev: hardcode to API port 8080.
    // Production: derive from current origin.
    const getWebSocketUrl = useCallback(() => {
        let baseUrl: string;
        if (typeof window !== 'undefined' && window.location.hostname === 'localhost') {
            // Local dev: API always on 8080
            baseUrl = 'http://localhost:8080';
        } else {
            baseUrl = client.getConfig().baseUrl || (typeof window !== 'undefined' ? window.location.origin : 'http://127.0.0.1:8000');
        }
